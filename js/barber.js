// barber.js - Barber Dashboard Functionality (jQuery Version)
...
async function checkBarberAccess() {
  try {
    ...
    if (!user || user.user_type !== 'barber') {
      alert('Access denied. Barber privileges required.');
      redirectToDashboard(user?.user_type);
      return;
    }
  } catch (err) {
    console.error('Barber check error:', err);
    window.location.href = 'login.html';
  }
}
...
async function loadBarberAppointments() {
  ...
  try {
    ...
    let url = `${API_URL}/barber/appointments`;
    ...
    if ($container.length) {
      $container.html(`<div class="error-state"><div class="error-icon">⚠</div><p>Error loading appointments</p><p class="error-details">${err.message}</p><button class="btn-primary" onclick="loadBarberAppointments()">Retry</button></div>`);
    }
  }
}
...
async function markAppointmentComplete(id) {
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/barber/appointments/${id}/complete`, {
      method: 'PATCH',
      headers: {
    ...
    loadBarberAppointments(); // Reload the list
  } catch (err) {
...
async function loadBarberProfile() {
  try {
    ...
    const res = await fetch(`${API_URL}/barber/profile`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    ...
    $('#name').val(barber.name || '');
    $('#email').val(barber.email || user.email || '');
    $('#phone').val(barber.phone || '');
    $('#specialty').val(barber.specialty || '');
    $('#bio').val(barber.bio || '');
    ...
    if ($profileImage.length) {
      if (barber.image_url) {
        $profileImage.attr('src', barber.image_url);
      } else if (user.avatar) {
...
async function updateBarberProfile(e) {
  ...
  try {
    ...
    loadBarberProfile(); // Reload the profile
  } catch (err) {
...
async function updateBarberCredentials(e) {
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/barber/credentials`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
...
async function uploadBarberImage(e) {
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/barber/profile/image`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`
      },
    ...
    console.error('Error uploading barber image:', err);
    if (toastId && typeof updateToast === 'function') {
