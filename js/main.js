    ...
    $.each(bookings, function(i, booking) {
      ...
      $card.html(`
        ...
        <div class="booking-card-body">
          ...
          <div class="booking-info-item">
            <span class="booking-label">Barber:</span>
            <span class="booking-value">${booking.barber?.name || 'N/A'}</span>
          </div>
          <div class="booking-info-item">
...
async function loadBarbers() {
  ...
  try {
    ...
    // Show skeleton loading
    if ($barbersGrid.length && typeof showSkeletonLoading === 'function') {
      showSkeletonLoading('barbersGrid', 4, 'card');
    }
    ...
    if (!res.ok) {
      ...
      let errorMessage = `Failed to load barbers (Status: ${res.status})`;
      try {
      ...
      if ($barbersGrid.length) {
        $barbersGrid.html(`<div class="error-state"><div class="error-icon">⚠</div><p>${errorMessage}</p><button class="btn-primary" onclick="loadBarbers()">Retry</button></div>`);
      }
    ...
    if (!Array.isArray(barbers)) {
      console.error('Invalid response format:', barbers);
      $barbersGrid.html('<div class="error-state"><div class="error-icon">⚠</div><p>Invalid response from server</p></div>');
      return;
    }

    if (barbers.length === 0) {
      $barbersGrid.html('<div class="empty-state"><p>No barbers found.</p></div>');
      return;
    }

    $.each(barbers, function(i, barber) {
      // Handle both specialty and specialization fields
      const specialty = barber.specialty || barber.specialization || 'Expert Barber';
      ...
      $card.html(`
        ...
        <div class="barber-info">
          ...
          ${barber.phone ? `<p class="barber-phone">📞 ${barber.phone}</p>` : ''}
        </div>
      `);
      $barbersGrid.append($card);
    });
  } catch (err) {
    console.error('Error loading barbers:', err);
    if ($barbersGrid.length) {
      $barbersGrid.html(`<div class="error-state"><div class="error-icon">⚠</div><p>Error loading barbers</p><p class="error-details">${err.message}</p><button class="btn-primary" onclick="loadBarbers()">Retry</button></div>`);
    }
  }
}
...
// Load Barber Options (for booking form)
async function loadBarberOptions() {
  ...
  try {
    ...
    // Show loading state in select
    if ($barberSelect.length) {
      $barberSelect.html('<option value="">Loading barbers...</option>').prop('disabled', true);
    }
    ...
    if ($barberSelect.length) {
      $barberSelect.html('<option value="">Select a barber...</option>').prop('disabled', false);

      $.each(barbers, function(i, barber) {
        const $option = $('<option>', {
          // Use barber_id for bookings if available (for compatibility with existing booking system)
          // Otherwise use user id
          value: barber.barber_id || barber.id,
          text: barber.name + (barber.specialty ? ` - ${barber.specialty}` : '')
        });
        $barberSelect.append($option);
      });

      // Show barber info on selection
      $barberSelect.off('change').on('change', function() {
        const $selectedOption = $(this).find('option:selected');
        const $barberInfo = $('#barberInfo');
        if ($selectedOption.val()) {
          // Find barber by barber_id or id
          const barber = barbers.find(b => (b.barber_id && b.barber_id == $selectedOption.val()) || b.id == $selectedOption.val());
          $barberInfo.html(barber && barber.bio ? `<em>${barber.bio}</em>` : '');
        } else {
          $barberInfo.html('');
        }
      });
    ...
    console.error('Error loading barbers:', err);
    if ($barberSelect.length) {
      $barberSelect.html('<option value="">Error loading barbers</option>').prop('disabled', false);
    }
    if (typeof showToast === 'function') {
...
async function loadAvailableSlots() {
  ...
  if (!barberId || !date) {
    $timeSelect.prop('disabled', true).html('<option value="">Select date and barber first...</option>');
    return;
  }
  ...
  try {
    ...
    const token = localStorage.getItem('authToken');
    const res = await fetch(`${API_URL}/available-slots?barber_id=${barberId}&date=${date}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
